...
## Five team members
Folders: `team/member-01/` through `team/member-05/`.
...
- Set `bCaption` and `bLabel` for the hover panel. Keep these short.
...
The member order is controlled by the `members` array in `index.json`. The current layout keeps five members, in aligned rows with deliberate blank space. Hover with a mouse, or tap/click/press Enter on a portrait to inspect the B-side. Escape or the close button dismisses it. The panel uses a translucent dark surface and a fine white outline in both themes.
